...
IGNORED_DIRECTORIES = {".git", "node_modules", "__pycache__", ".venv", "venv", ".tox", ".mypy_cache"}
...
ENDPOINT_PATTERN = re.compile(r"@(?:router|app)\.(get|post|put|delete|patch)\(\s*[\"']([^\"']+)[\"']")
...
AUTH_PATTERN = re.compile(r"\b(jwt|oauth|authenticate|login_required|require_admin|authorization)\b", re.IGNORECASE)
...
class ArchitectureSection:
    ...
    def observed_count(self) -> int:
        return sum(1 for finding in self.findings if finding.classification == "observed")

    @property
    def inferred_count(self) -> int:
        return sum(1 for finding in self.findings if finding.classification == "inferred")
...
def architecture_draft_paths(repo_path: str, provider: str, draft_id: str) -> tuple[str, str]:
    """Returns the (rst_path, json_metadata_path) artifact locations for a draft id."""
    rst_path = build_repo_output_file(repo_path, provider, f"architecture_draft_{draft_id}.rst")
    json_path = build_repo_output_file(repo_path, provider, f"architecture_draft_{draft_id}.json")
...
def _iter_source_files(root_dir: str, target_folders: list[str]) -> list[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir):
        ...
        for filename in filenames:
            ...
            if normalized_targets and not _file_matches_target_folders(rel_path, normalized_targets):
                continue
...
def _scan_repository(root_dir: str, target_folders: list[str]) -> dict:
    ...
    top_level_packages = {
        ...
        if os.path.isdir(os.path.join(root_dir, name)) and name not in IGNORED_DIRECTORIES
    }
    ...
    for rel_path in scan_paths:
        ...
        if ext in SOURCE_EXTENSIONS:
            ...
            evidence["language_counts"][language] = evidence["language_counts"].get(language, 0) + 1
        ...
        for method, path in ENDPOINT_PATTERN.findall(content):
            evidence["endpoints"].append({"method": method.upper(), "path": path, "file": rel_path})
        if any(hint in content for hint in MODEL_BASE_HINTS) and rel_path not in evidence["models"]:
            evidence["models"].append(rel_path)
...
def _paths_section(
    ...
    findings = [
        _finding(finding_type, path, "observed", f"File path matched {section_name} detection heuristics.", [path])
...
def _parse_external_dependencies(root_dir: str, dependency_files: list[str]) -> list[tuple[str, str, str]]:
...
def _build_sections(
    ...
    overview_findings = [_finding("technology", "repository scan", "observed", overview_summary, [])]
    sections.append(
        ArchitectureSection(
            "project overview",
    ...
    if external_deps:
        ...
        findings = [
            _finding("dependency", name, "observed", f"Declared in {manager_label} manifest.", [source_file])
    ...
    if len(flow_stages) >= 2:
        ...
        sections.append(
            ArchitectureSection(
                "data flow", "unavailable", "No data flow evidence found.", [], "not_applicable", [data_flow_gap]
            )
        )
    ...
    if evidence["endpoints"]:
        ...
        sections.append(
            ArchitectureSection(
                "API endpoints", "unavailable", "No API endpoint evidence found.", [], "not_applicable", [endpoints_gap]
            )
        )
    ...
    if include_diagrams and len(flow_stages) >= 2:
        confidence = _confidence_from_count(len(flow_stages) * 2)
        diagram_content["architecture"] = " -> ".join(stage.replace(" ", "_") for stage in flow_stages)
        diagram_content["sequence"] = "\n".join(
            f"{flow_stages[index]} -> {flow_stages[index + 1]}: call" for index in range(len(flow_stages) - 1)
        )
        sections.append(
            ArchitectureSection(
                "architecture diagrams",
        ...
        if not include_diagrams:
            ...
            reason = "Not enough layered evidence was found to generate a reliable diagram."
            gap_type = "ambiguous"
        for diagram_section_name in ("architecture diagrams", "sequence diagrams"):
            ...
            sections.append(
                ArchitectureSection(diagram_section_name, "unavailable", reason, [], "not_applicable", [diagram_gap])
            )

    # repository structure
    if evidence["top_level_entries"]:
        structure_findings = [
            _finding("module", entry, "observed", "Top-level repository entry.", [entry])
    ...
    # technology stack
    tech_findings = []
    for language, count in sorted(evidence["language_counts"].items(), key=lambda item: -item[1]):
    ...
    for rel_path in evidence["dependency_files"]:
        manager_label = DEPENDENCY_FILES.get(os.path.basename(rel_path), os.path.basename(rel_path))
        tech_findings.append(
            _finding("technology", manager_label, "observed", "Detected from dependency manifest file.", [rel_path])
        )
    if tech_findings:
        sections.append(
            ArchitectureSection(
...
def render_architecture_draft_rst(
    ...
    for section in sections:
        ...
        lines.append("")
        lines.append(f"Status: {section.status} | Confidence: {_confidence_label(section.confidence_level)}")
        lines.append("")
        ...
        for finding in section.findings:
            ...
            evidence = f" [{', '.join(finding.evidence_paths)}]" if finding.evidence_paths else ""
            lines.append(f"- **{finding.name}** -- {marker}: {finding.description}{evidence}")
        if section.findings:
            lines.append("")
        for gap in section.gaps:
            ...
            lines.append("")
        if section.section_name == "architecture diagrams" and diagram_content.get("architecture"):
            lines.append(".. code-block:: text")
            lines.append("")
            for diagram_line in diagram_content["architecture"].splitlines():
                lines.append(f"   {diagram_line}")
            lines.append("")
        if section.section_name == "sequence diagrams" and diagram_content.get("sequence"):
            lines.append(".. code-block:: text")
            lines.append("")
...
def generate_architecture_draft(
    ...
    logger.info("Analyzing architecture for repo: provider=%s, url=%s, branch=%s", provider, repo_url, branch)
    ...
    try:
        with clone_repository(repo_url, token, branch, provider) as temp_dir:
            ...
            evidence["_external_deps"] = _parse_external_dependencies(temp_dir, evidence["dependency_files"])
    except RepositoryAccessError as exc:
        raise ArchitectureAnalysisError(str(exc), status_code=exc.status_code or 404) from exc
    ...
    if reuse_existing_docs:
        existing_content = fetch_existing_output_content(repo_path, branch, normalized_output_path, token, provider)
    ...
    draft_content = render_architecture_draft_rst(project_name, sections, diagram_content)

    draft_id = generate_draft_id()
    draft_rst_path, draft_json_path = architecture_draft_paths(repo_path, provider, draft_id)
    ...
    navigation_update = propose_architecture_navigation(repo_path, branch, token, provider, normalized_output_path)

    overall_status = "success"
    if evidence["truncated"] or navigation_update.get("conflict"):
        overall_status = "partial"
    critical_sections = {"entry points", "repository structure"}
    if any(section.status == "unavailable" for section in sections if section.section_name in critical_sections):
        overall_status = "partial"
...
def find_architecture_draft(repo_path: str, provider: str, draft_id: str) -> dict | None:
    ...
    run_dirs = sorted(
        (os.path.join(repo_base, entry) for entry in os.listdir(repo_base) if entry.startswith("app_")),
        reverse=True,
    )
...
def apply_architecture_approval(
    ...
    if draft_metadata is None:
        raise ArchitectureApprovalError(f"Architecture draft '{draft_id}' was not found for this repository.")
    ...
    if not os.path.exists(draft_path):
        raise ArchitectureApprovalError(f"Architecture draft content for '{draft_id}' is no longer available.")
    with open(draft_path, "r", encoding="utf-8") as handle:
        draft_content = handle.read()

    existing_content = fetch_existing_output_content(repo_path, branch, normalized_output_path, token, provider)
    if existing_content is not None and not is_autodoc_generated_content(existing_content) and not overwrite_existing:
    ...
    if not navigation_applied:
        logger.warning("Architecture doc approved for draft '%s' but navigation update failed.", draft_id)
